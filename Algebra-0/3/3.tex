...
\begin{problem}[3D-13]
    Show that the result in Exercise 3D-12 can fail without the hypothesis that \( V \) is finite-dimensional.
\end{problem}

\begin{proof}
    Take \( V=\F^\infty \), and 
    \[ S(a_1,a_2,a_3,\cdots)=(a_3,\cdots),\ T=U,\ U(a_1,a_2,\cdots) =(0,a_1,a_2,\cdots). \]
    It's easy to verify that \( STU=I \), but \( T \) is not invertible since \( \im T\neq\F^\infty \). 
\end{proof}
