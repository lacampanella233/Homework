...
\begin{proof}
  ...
  If \( \lambda \) is not an eigenvalue of \( T \), then \( T-\lambda I \) is injective in \( V \), as well as in \( U \) (because \( U \) is T-invariant). Then there exists \( u_0\in U \) such that \( Tu_0-\lambda u_0=u\in U \). \par
...
\begin{solution}
  (a) Let \( S=\begin{pmatrix}
    0 & 0 \\
    1 & 0
  \end{pmatrix} \) and \( T=\begin{pmatrix}
    0 & 1 \\
    0 & 0
  \end{pmatrix} \). Then \( ST=0 \) and \( TS=\begin{pmatrix}
    1 & 0 \\
    0 & 0
  \end{pmatrix} \). Hence, the minimal polynomial of \( ST \) is \( x \), while the minimal polynomial of \( TS \) is \( x(x-1) \).
  \par
...
\begin{proof}
  Take \( v_k=(0,\dots,0,\overset{k \text{-th}}{1},0,\dots,0)\transpose \) where the $1$ is in the $k$-th position. Then from the representation matrix of \( T \), we can infer that
  \[ Tv_1=v_2, \  Tv_2=v_3, \  \ldots, \  Tv_{n-1}=v_n, \  Tv_n=-a_0v_1-a_1v_2-\cdots-a_{n-1}v_{n-1} \]
  Denote the minimal polynomial of \( T \) by \( m(x) \). If \( \deg m<n \), then \( m(T)(v_1) \) is a linear comnbinition of \( v_1,\dots,v_{n} \) with not-all-zero coefficients, which contradicts the definition of \( m \). Hence, \( \deg m\ge n \). On the other hand, it's easy to verify that
  \[ T^n v_1=-a_0v_1-a_1v_2-\cdots-a_{n-1}v_{n-1}\implies \left( \sum_{k=0}^{n-1}a_kT^k+T_n \right)(v_1)=0. \]\par
  Denote \( p(x)=\sum_{k=0}^{n-1}a_kx^k+x^n \), then \( \deg p=n \) and \( p(T)v_1=0 \). And similarly, \( p(T)v_k=p(T)T^{k-1}(v_1)=0 \). Therefore, \( p(T)=0 \), which indicates \( p(x)=m(x) \).
\end{proof}
...
\begin{proof}
  The minimal polynomial of \( T \) is a product of powers of \( x-3, x-5, x-8 \), say \( m(x)=(x-3)^{\alpha_1}(x-5)^{\alpha_2}(x-8)^{\alpha_3} \), where \( \alpha_1,\alpha_2,\alpha_3\in\N_+ \). Note that \( \deg m\le 4 \), hence \( \alpha_1\le 4-1-1=2 \), which means \( (x-3)^{\alpha_1}\left|(x-3)^2\right. \). The similar argument applies to \( (x-5)^{\alpha_2} \) and \( (x-8)^{\alpha_3} \). Therefore, \( m(x)\left|\,(x-3)^2(x-5)^2(x-8)^2\right. \), which implies that \( (T-3I)^2(T-5I)^2(T-8I)^2=0 \).
\end{proof}



\begin{problem}[5B-21]
Suppose $V$ is finite-dimensional and $T \in \mathcal{L}(V)$. Prove that the minimal polynomial of $T$ has degree at most $1 + \dim\im T$.
\end{problem}

\begin{proof}
  Denote the minimal polynomial of \( T \) by \( m_T(x) \). Note that \( m_T(x) \) divides the characteristic polynomial of \( T \) which has degree at most \( \dim V \). Apply the same argument to \( T|_{\im T} \), we have \( \deg m_{T|_{\im T}}\le\dim\im T \). \par
  For any vector \( v\in V \), \( T(v)\in\im T \), which means \( m_{T|_{\im T}}(T(v))=0 \). Therefore, \( m_T(x) \) divides \( m_{T|_{\im T}}(x)x \), and hence \( \deg m_T(x)\le 1 + \dim\im T \).
\end{proof}
...
\begin{proof}
  Let \( n \) be the dimension of \( V \). On the one hand, Suppose that
  \[ I=a_1T+a_2T^2+\dots+a_nT^n. \]
  Set \( S=a_1I+a_2T+\dots+a_nT^{n-1} \). Then \( ST=TS=a_1T+a_2T^2+\dots+a_nT^n=I \), which means \( S=T^{-1} \).\par
  On the other hand, if \( T\notin\sspan \left\{ T,T^2,\dots,T^n \right\} \), then take the minimal polynomial of \( T \), say \( m_T(x) \). Then we have \( m_T(0)=0 \), i.e.
  \[ m_T(x)=a_1x+a_2x^2+\dots+a_dx^d \]
  for some \( d\le n \) and \( a_1,\dots,a_d\in\F \). Since \( a_1I+a_2T+\dots+a_dT^{d-1} \) is not zero (because \( m_T(x) \) is the minimal polynomial), there exists \( u,v\neq0 \) such that
  \[ u=(a_1I+a_2T+\dots+a_dT^{d-1})(v). \]
  Then \( T(u)=0 \) and \( u\neq 0 \), and conseqently \( T \) is not invertible.
\end{proof}
...
\begin{proof}
  (a) It suffices to show that \( m_T(T/U)=0 \). For any \( v+U\in V/U \), we have \( m_T(T/U)(v+U)=m_T(T)(v)+U=0+U=0 \). Hence, \( m_T(T/U)=0 \), which implies that \( m_T(x) \) is a polynomial multiple of the minimal polynomial of \( T/U \).\par
...
\begin{note}
  We will only consider the situation where the corresponding field is \( \R \) or \( \C \). 
\end{note}

\begin{proof}
  Denote the linear transformation by \( T\in \mathcal{L}(V) \), and its minimal polynomial by \( m(x) \). \par
  We will prove the problem by induction on \( \dim V=:n \). The statement obviously holds for \( n=2 \). Assume that the statement holds for any vector space of dimension less than \( n \). We will show that it also holds for \( V \).\par
  If \( m(x) \) has two or more relatively prime factors, then \( m(x) \) can be decomposed as \( m(x)=m_1(x)m_2(x) \), where \( \gcd(m_1,m_2)=1 \). Denote \( U_1=\ker m_1(T), U_2=\ker m_2(T) \). Then \( V=U_1\oplus U_2 \), and \( U_{1,2}\neq V \), hence \( 1\le\dim U_{1,2}\le n-1 \). Since \( n\ge 3 \), either \( \dim U_1\ge 2 \) or \( \dim U_2\ge 2 \). Then, use the induction hypothesis on \( T|_{U_i} \), where \( \dim U_i\ge 2,\ i\in \left\{ 1,2 \right\} \). \par
  Otherwise, \( m(x) \) has the form \( p(x)^\alpha \), where \( p(x) \) is irreducible on \( \F[x] \).
  \begin{itemize}
    \item If \( \F=\R \) and \( \deg p(x)=2 \). Since \( p(T)^{\alpha-1} \) is not zero, there exists \( v\in V \) such that \( p(T)^{\alpha-1}(v)\neq 0 \). Let's prove that \( U=\sspan\left\{ p(T)^{\alpha-1}(v),p(T)^{\alpha-1}T(v) \right\} \) is the desired invariant subspace of dimension two. Otherwise there exists \( \lambda\in\R \) such that \( p(T)^{\alpha-1}T(v)=\lambda p(T)^{\alpha-1}(v) \). Then
    ...
    \item If \( \deg p(x)=1 \). Then \( T \) has an eigenvector, denoted as \( v_1 \). Let \( W=\sspan \{v_1\} \). Since \( m_{T/W}(x) \) divides \( m_T(x) \), then \( m_{T/W}(x) \) is also a power of \( p(x) \), which implies that \( T/W \) has an eigenvector, denoted as \( v_2+W \). Then \( v_1,v_2 \) are linearly independent, and \( U=\sspan\{v_1,v_2\} \) is the desired invariant subspace of dimension two.
  \end{itemize}
\end{proof}
