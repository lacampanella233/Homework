...
\begin{proof}
	...
	根据定义, $x\in A$. 对于每一个 $n \ge 1$, 均有 $a_n^{(n)}\neq b_n$, 即 $x_n$ 与 $x$ 的十进制小数展开中均有至少一位不相同. 而 $A$ 中的每一个数均有唯一的十进制小数展开 (若一个实数有两个不同的十进制小数展开, 那么必定有一个展开为循环节为 $9$ 的混循环小数, 而另一个展开是有限小数), 从而得到了矛盾.
...
\begin{proof}
	...
	对于每一个 $1\le k\le n$, 我们均构造了一个闭区间套 $[a_k^{(0)},b_k^{(0)}] \supset [a_k^{(1)},b_k^{(1)}] \supset [a_k^{(2)},b_k^{(2)}] \supset\dots$, 并且这个闭区间套中的闭区间长度收敛于 $0$, 因此存在唯一的公共元素 $x_k$. 考虑点 $x=(x_1,\dots,x_k)\in \R^n$. 根据构造, 我们可以选取 $x^{(m)}\in K\cap\prod_{k=1}^{n} [a_k^{(m)},b_k^{(m)}]$ (后者显然非空, 否则其有可数子覆盖), 而 $\lim_{ m \to \infty } x^{(m)}=x$, 结合 $K$ 为闭集, 知 $x\in K$. 
...
\begin{proof}
	...
	接下来证明原命题. 采用反证法. 否则, 存在两个 $f(E)$ 中的两个不交的开集 $A,B$, 使得 $f(E) = A\cup B$. 则 $E = f|_E^{-1}(A) \cup f|_E^{-1}(B)$, 且后两者为 $E$ 中的不交的开集, 这与 $E$ 是连通集矛盾. 因此 $f(E)$ 是连通集.
\end{proof}
...
\begin{solution}
	...
	考虑嵌套的闭球序列 $\overline{B_{r_1/2}(y_1)} \supset \overline{B_{r_2/2}(y_2)} \supset \dots$, 它们的交集非空, 不妨设其中的一点为 $y$. 则对任意的 $n$, 有 $y \in \overline{B_{r_n/2}(y_n)} \in B_{r_n}(y_n) \subset U_n\cap B_r(x)$, 因此
