...
\item 若$ z<w $, 则$ a<c $, 此时不可能有$ z=w $或$ w<z $; 或者$ a=c $(因此不可能有$ w<z $)且$ b<d $(因此不可能有$ z=w $).\par 
...
\item 证明: 若$ z,w,u\in\mathbb{C} $, 且$ z<w,\ w<u $, 则$ z<u $. \par
由$ z<w $知$ a<c $, 或$ a=c $且$ b<d $; 由$ w<u $知$ c<e $, 或$ c=e $且$ d<f $ (此处记$ u=e+f\i $). \par
\end{enumerate}
综上所述, $ z<w $是一个序关系. \par
(2) 证明配备了这个序关系的$ \mathbb{C} $没有最小上界性. 设$ A=\{z=a+b\i:\ b<0 \} $. 则$ A $的上界为$ 0 $, 但$ A $没有最小上界, 因为对于任意的实数$ x=x+0\i $, $ x $均为$ A $的上界. 因此, $ A $没有最小上界.
\end{solution}
